        ...
        for epoch in range(1, epochs + 1):
            ...
            for batch_idx, (input_ids, attention, labels) in enumerate(train_loader, start=1):
                ...
                if scaler.is_enabled():
                    # Scaled backward + unscale before gradient clipping
                ...
                if log_interval > 0 and (global_step % log_interval == 0) and rank == 0:
                    ...
                    # Step-level logs: accuracy is not computed here → store None (JSON -> null)
                    run_loss_step.append(avg_train_loss)
                    run_acc_step.append(None)
                    ...
                    print(" | ".join(
                        ...
                        for v in (
                            f"{run}*",
                            f"{epoch + batch_idx / len(train_loader):.2f}",
                            avg_train_loss,
                            "N/A",          # no accuracy for mid-epoch log
                            elapsed_mid,
                        )
                    ))


            # End-of-epoch metrics (full val)
            epoch_loss /= max(1, nb)
...
def main():
    ...
    parser.add_argument("--log_interval", type=int, default=0,
                        help="If >0, log mid-epoch train loss every N steps (no full val).")
